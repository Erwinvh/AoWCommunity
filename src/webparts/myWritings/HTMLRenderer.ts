...
export default class HTMLRenderer{
    ...
    //Private render part method to construct the my writing view.
    //Users will be able to see the writings they have written in this view.
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the my writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
    ...
    //Private render part method to construct the header for the my writing view.
    //The header allows the user to search through their own writings easier through the sort by bar and the search bar.
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the header of the my writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
    // - TODO:      The sort by and search bar have no function as of now.
    ...
    //Private render part method to construct the category dropdown areas for the my writing view.
    // - Parameter: Category: String, is a string with 1 of the core 5 categories: "Fiction", "Nonfiction", "Poetry", "Scripts", "Other"
    // - Returns:   A string containing the HTML code for the category dropdown area of the new writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called multiple times per instance of the webpart.
    private RenderCatagorieOwnedWriting(Category: string):string{
        let html:string = `
            <button type="button" class="${styles.collapsible}" style="`
            switch(Category){
                case "Fiction":
    ...
    //Public render method to construct the new writing view.
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the new writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
    ...
    //Private render part method to construct the header for the new writing view.
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the header of the new writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
    ...
    //Private render part method to construct the container for the new writing insertion fields in the new writing view.
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the container of the insertion fields for a new writing for the new writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
    ...
    //Private render part method to construct the base fields related to the insertion of a new writing piece in the new writing view.
    //The user will be able to insert information about a new writing within these fields. Information such as a description and title.
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the insertion fields for a new writing for the new writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
    // - TODO:      Some of the data in the fields doesnt get reset when using the back button, this needs to be fixed.
    private RenderBaseInfoFields():string{
        ...
        <div id="BaseInfoFields">
            ...
            <h3>Feedback</h3>
            <div class="${styles.row}">
                <label class="${styles.col25}" for="AllFeedback">Allow feedback</label>
                <div class="${styles.col75}">
                    <input class="${styles.inputBox}" type="checkbox" id="AllFeedback" name="AllFeedback">
                </div>
            </div>
            <div id="FeedbackFieldsArea" style="display:none;">
                <div class="${styles.row}">
                    <label class="${styles.col25}" for="VisibleFeedback">Allow feedbackers to see each others feedback</label>
                    <div class="${styles.col75}">
                        <input class="${styles.inputBox}" type="checkbox" id="VisibleFeedback" name="VisibleFeedback">
                    </div>
                </div>
                <div class="${styles.row}">
                    <label class="${styles.col25}" for="FormalFeedback">Allow feedbackers to give formal feedback</label>
                    <div class="${styles.col75}">
                        <input class="${styles.inputBox}" type="checkbox" id="FormalFeedback" name="FormalFeedback">
                    </div>
                </div>
                <div class="${styles.row}">
                    <label class="${styles.col25}" for="CommentsFeedback">Allow feedbackers to leave comments</label>
                    <div class="${styles.col75}">
                        <input class="${styles.inputBox}" type="checkbox" id="CommentsFeedback" name="CommentsFeedback">
                    </div>
                </div>
                <div class="${styles.row}">
                    <label class="${styles.col25}" for="QAFeedback">Allow feedbackers to answer your questions</label>
                    <div class="${styles.col75}">
                        <input class="${styles.inputBox}" type="checkbox" id="QAFeedback" name="QAFeedback">
                    </div>
                </div>
                <div id="NewQuestionFieldArea" style="display:none;">
                    <div class="${styles.row}">
                        <label class="${styles.col25}" for="setQuestionsField">your feedback questions</label>
                        <div class="${styles.col75}">
                            <div class="${styles.GTTGridContainer}">
                                <div>
                                </div>
                                ...
                                <div id="excludedTagsSingle">
                                    <input class="${styles.inputBox}" type="text" id="NewQuestionField" name="NewQuestionField" placeholder="New feedback question..">
                                    <button id="addNewQuestionButton">Add</button>
                                </div>

                            <div id="ControlsTagsSingle">
                                <div id="SelectedQuestion" hidden></div>
                                <button id="removeQuestionButton">Remove</button>
                            </div>
                            <div id="includedQuestions" class="${styles.GTTScrollContainer}">
                ...
                <div class="${styles.row}">
                    <label class="${styles.col25}" for="InlineFeedback">Allow feedbackers to make inline notes</label>
                    <div class="${styles.col75}">
                        <input class="${styles.inputBox}" type="checkbox" id="InlineFeedback" name="InlineFeedback">
                    </div>
                </div>
            </div>
            ...
            <div class="${styles.row}">
                ...
                <div class="${styles.col75}">
                    <label for="NewDocument" id="NewDocumentDropArea" class="${styles.dropArea}">
                    <input type="file" accept=".pdf" id="NewDocument" name="filename" hidden>
                    <div class="${styles.uploadImage}">
                        <img id="NewDocumentImage" src="${require('../../shared/assets/upload.png')}">
                        <p id="NewDocumentTag">Click here to upload your .pdf file</p>
                        <span id="NewDocumentInstruction">Upload any .pdf files from desktop</span>
                    </div>
                    </label>
                </div>
            </div>
    ...
    //Private render part method to construct the fields related to the insertion of a new version of an existing writing piece in the new writing view.
    //The user will be able to insert information about a new version of an existing writing piece within these fields. 
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the new version of an existing writing insertion fields of the new writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
    // - TODO:      The selected collection doesn't result a change in the available singles. (Due to error in time estimations)
    ...
    //Private render part method to construct the fields related to the insertion of a new collection in the new writing view.
    //The user will be able to insert information about a new collection within these fields while inserting information about a new writing. 
    // - Parameter: None
    // - Returns:   A string containing the HTML code for the collection insertion fields of the new writing view.
    // - Example:   This method is called when making a new My writing webpart and should only be called once per instance of the webpart.
